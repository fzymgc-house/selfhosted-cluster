...
A comprehensive GitOps-managed Kubernetes cluster setup for home infrastructure using k3s, FluxCD, and Terraform. This repository provides Infrastructure as Code (IaC) for a complete self-hosted platform including identity management, monitoring, databases, and application deployment.
...
- **GitOps**: FluxCD v2.6.x for declarative cluster management
...
├── fluxcd/                     # GitOps manifests and configurations
│   ├── clusters/main/          # Cluster-level FluxCD resources
│   ├── infrastructure/         # Infrastructure controllers and configs
│   │   ├── controllers/        # Core infrastructure components
│   │   └── configs/            # Cluster-wide configurations
│   └── apps/main/              # Application deployments
...
└── containers/                 # Custom container images
    └── fluxcd-tofu-runner/     # Custom Terraform runner for FluxCD
```
...
- **🏗️ Terraform (via FluxCD)**: External service configuration
...
### GitOps with FluxCD
The `fluxcd/` directory implements GitOps workflows:

- **Infrastructure Controllers**: Deploy core Kubernetes controllers
...
2. **Automated Sync**: FluxCD monitors repository changes
3. **Declarative Updates**: Kubernetes resources reconciled automatically
4. **Rollback Capability**: Git-based change history

### Development Process
1. Make changes to manifests in feature branches
2. Create pull request for review
3. Merge to main branch triggers deployment
4. FluxCD applies changes to cluster automatically
...
### Troubleshooting
- Check FluxCD status: `flux get all`
- View logs: `kubectl logs -n flux-system -l app=source-controller`
- Validate manifests: `kubectl apply --dry-run=client -k fluxcd/clusters/main/`
