...
| Vault | Stores DB credentials and OIDC credentials |
...
Note: OIDC credentials are managed by Terraform (`tf/teleport/`) which reads directly from Vault.
Join tokens are generated dynamically by Ansible using `tctl tokens add`.
...
| `secret/fzymgc-house/cluster/teleport/oidc` | Authentik client ID and secret |

Note: Join tokens are no longer stored in Vault. They are generated dynamically using `tctl tokens add` with short TTLs.
...
- **DB credentials:** Create manually in Vault before deployment
- **OIDC client secret:** Created automatically by Authentik Terraform module
...
4. Enable and start `teleport` systemd service

**Join Token Generation:**
The playbook generates short-lived tokens dynamically using `tctl tokens add --type=node --ttl=15m`.
This eliminates the need for static tokens stored in Vault and improves security.
...
- Deploy agents to all 4 nodes (tokens generated dynamically)
